...
export default function Page() {
  const schemas = [
    { '@context': 'https://schema.org', '@type': 'Organization', name: siteConfig.name, url: siteConfig.url, logo: `${siteConfig.url}${siteConfig.logo}` },
    { '@context': 'https://schema.org', '@type': 'BreadcrumbList', itemListElement: [{ '@type': 'ListItem', position: 1, name: 'Home', item: siteConfig.url }, { '@type': 'ListItem', position: 2, name: 'Careers', item: `${siteConfig.url}/careers` }] },
    ...jobs.map(job => ({ '@context': 'https://schema.org', '@type': 'JobPosting', title: job.title, description: job.summary, employmentType: 'FULL_TIME', hiringOrganization: { '@type': 'Organization', name: siteConfig.name, sameAs: siteConfig.url, logo: `${siteConfig.url}${siteConfig.logo}` }, jobLocation: { '@type': 'Place', address: { '@type': 'PostalAddress', addressRegion: 'Tamil Nadu', addressCountry: 'IN' } }, experienceRequirements: job.experience, industry: 'Environmental Biotechnology and Wastewater Treatment' })),
  ]
