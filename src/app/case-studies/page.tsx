...
import { caseStudies } from "@/data/resource-content";
import { createMetadata, siteConfig } from "@/lib/site";

export const metadata: Metadata = createMetadata({
  title: "Industrial Wastewater Case Studies | Bionics",
  description:
    "Review representative Nanozyme wastewater treatment case studies covering textile ETP, distillery anaerobic digestion and dairy organic-load improvement.",
  path: "/case-studies",
  keywords: [
      ...
      <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10">
        ...
        <header className="max-w-4xl py-10 lg:py-12">
          ...
          <p className="mt-5 text-lg leading-8 text-slate-600">
            Concise representative reports covering the treatment challenge,
            implementation approach and reviewed plant performance.
          </p>
        </header>
        <section className="grid gap-6 lg:grid-cols-3">
          {caseStudies.map((study) => (
            ...
            >
              <Image
                src={study.image}
                alt={`${study.industry} wastewater treatment case study`}
                width={720}
                height={430}
                loading="lazy"
              ...
              <div className="flex flex-1 flex-col p-5 sm:p-6">
                <p className="text-xs font-bold uppercase tracking-wider text-emerald-700">
                  {study.industry} · {study.capacity}
                </p>
                <h2 className="mt-2 text-2xl font-black text-slate-950">
                  {study.title}
                </h2>
                <p className="mt-4 text-sm leading-6 text-slate-600">
                  <strong className="text-slate-900">Challenge:</strong>{" "}
                  {study.problem}
                </p>
                <p className="mt-3 text-sm leading-6 text-slate-600">
                  <strong className="text-slate-900">Nanozyme:</strong>{" "}
                  {study.nanozyme}
                </p>
                ...
                <details className="mt-4 rounded-xl border border-slate-200 p-3 text-sm">
                  <summary className="cursor-pointer font-bold text-cyan-800">
                    Implementation details
                  </summary>
                  <p className="mt-3 leading-6 text-slate-600">
                    {study.implementation}
                  </p>
                  <blockquote className="mt-3 border-l-2 border-cyan-400 pl-3 italic text-slate-600">
                    “{study.feedback}”
                  </blockquote>
                </details>
                <div className="mt-auto flex flex-wrap gap-2 pt-5">
                  <span className="inline-flex items-center gap-2 rounded-full border border-slate-300 px-3 py-2 text-xs font-bold text-slate-500">
