...
export default function CardGrid({ eyebrow, title, description, items, columns = 3 }: CardGridProps) {
    ...
    return (
        <section className="mx-auto max-w-7xl px-4 py-5 sm:px-8 lg:px-10 lg:py-8">
            ...
            <div className={`mt-5 grid gap-3 sm:mt-6 sm:gap-4 ${className}`}>
                {items.map((item) => {
                    const Icon = item.icon
                    return (
                        <div key={item.title} className="flex min-h-[88px] items-start gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition duration-300 hover:-translate-y-1 hover:border-emerald-200 hover:shadow-lg sm:min-h-[96px] sm:items-center">
