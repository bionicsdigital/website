...
export default function CardGrid({ eyebrow, title, description, items, columns = 3 }: CardGridProps) {
    ...
    return (
        <section className="mx-auto max-w-7xl px-5 py-16 sm:px-8 lg:px-10 lg:py-20">
            ...
            <div className={`mt-10 grid gap-6 ${className}`}>
                {items.map((item) => {
                    const Icon = item.icon
                    return (
                        <div key={item.title} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
