...
function mark(text: string, query: string) {
  ...
  const escaped = query.trim().split(/\s+/).filter(Boolean).map((term) => term.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')
  if (!escaped) return text
  const parts = text.split(new RegExp(`(${escaped})`, 'ig'))
  return parts.map((part, index) => new RegExp(`^(?:${escaped})$`, 'i').test(part) ? <mark key={index} className="rounded bg-emerald-100 text-emerald-950">{part}</mark> : part)
}
...
export default function SearchExperience({ compact = false, onNavigate }: { compact?: boolean; onNavigate?: () => void }) {
  ...
  return <div className={compact ? 'flex h-full min-h-0 flex-col' : ''}>
    ...
    {!query && <div className={`mt-4 shrink-0 ${compact ? 'max-sm:hidden' : ''}`}><p className="text-xs font-bold uppercase tracking-wider text-slate-400">{recent.length ? 'Recent searches' : 'Popular searches'}</p><div className="mt-2 flex flex-wrap gap-2">{(recent.length ? recent : popular).map((item) => <button key={item} onClick={() => setQuery(item)} className="rounded-xl bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-emerald-50 hover:text-emerald-800">{item}</button>)}</div></div>}
    <div className={`mt-4 space-y-2 overflow-y-auto overscroll-contain pr-1 ${compact ? 'min-h-0 flex-1' : ''}`} role="listbox" aria-label="Search results" aria-live="polite">{results.length ? results.map((item: SearchItem) => { const Icon = icons[item.category]; return <Link key={item.id} href={item.href} onClick={() => { remember(query || item.title); onNavigate?.() }} className={`group flex items-start gap-3 rounded-2xl border border-slate-100 bg-white transition hover:border-cyan-200 hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500 ${compact ? 'p-3' : 'p-3.5'}`} role="option"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-50 to-cyan-50 text-emerald-700"><Icon className="h-5 w-5" /></span><span className="min-w-0 flex-1"><span className="block font-bold text-slate-950">{mark(item.title, query)}</span><span className={`${compact ? 'line-clamp-1' : 'line-clamp-2'} mt-0.5 block text-xs leading-5 text-slate-500`}>{item.description}</span><span className="mt-1 block text-[10px] font-bold uppercase tracking-wider text-emerald-600">{item.category}</span></span><ArrowRight className="mt-3 h-4 w-4 text-slate-300 transition group-hover:translate-x-1 group-hover:text-cyan-500" /></Link> }) : <div className="rounded-2xl border border-dashed border-slate-300 p-8 text-center"><Search className="mx-auto h-8 w-8 text-slate-300" /><h2 className="mt-3 font-bold text-slate-950">No results found</h2><p className="mt-1 text-sm text-slate-500">Try a broader term or another filter.</p></div>}</div>
  </div>
}
