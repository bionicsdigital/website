...
export default function BackgroundParticles() {
  const [count, setCount] = useState(80); const viewportWidth = useMotionValue(0); const viewportHeight = useMotionValue(0); const reduceMotion = useReducedMotion(); const { pointerX, pointerY } = usePointer(); const scrollY = useScrollMotion()
  ...
  return <div aria-hidden="true" className="pointer-events-none fixed inset-0 z-[1] overflow-hidden"><div className="absolute inset-0 opacity-80">{PARTICLES.slice(0, count).map((particle) => <Particle key={particle.id} particle={particle} pointerX={pointerX} pointerY={pointerY} scrollY={scrollY} viewportWidth={viewportWidth} viewportHeight={viewportHeight} reduceMotion={Boolean(reduceMotion)} />)}</div></div>
}
